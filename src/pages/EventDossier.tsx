...
import { useCustomTemplate, useSaveTemplate, useAiFillTemplate, getDefaultTemplate, TemplateStructure, TemplateType } from "@/hooks/useCustomTemplates";

type View = "client" | "manager";
type PageMode = "document" | "editor";
...
const EventDossier = () => {
  ...
  // Template state
  const currentTemplateType: TemplateType = view === "client" ? "client" : "traiteur";
  const { data: savedTemplate } = useCustomTemplate(currentTemplateType);
  const saveTemplate = useSaveTemplate();
  const aiFill = useAiFillTemplate();

  const [templateData, setTemplateData] = useState<TemplateStructure | null>(null);

  // Load template when type or saved template changes
  useEffect(() => {
    if (savedTemplate) {
      setTemplateData(savedTemplate.structure as unknown as TemplateStructure);
    } else {
      setTemplateData(getDefaultTemplate(currentTemplateType));
    }
  }, [savedTemplate, currentTemplateType]);

  // Editable fields (legacy — kept for backward compat)
  ...
  useEffect(() => {
    if (!selectedEventId) {
      setSelectedEvent(null); setQuote(null); setStaff([]); setClient(null);
      return;
    }
    ...
    supabase.from("quotes").select("*").eq("event_id", selectedEventId).order("created_at", { ascending: false }).limit(1)
      .then(({ data }) => {
        ...
        if (q) {
          ...
          setMenuDescription(items.map((it) => `• ${it.name} (x${it.qty})`).join("\n") || "Menu à définir.");
        } else {
  ...
  const buildDataset = () => ({
    event: selectedEvent,
    quote: quote ? { items: quoteItems, subtotal, tva_rate: tvaRate, total_ttc: totalTtc, notes: quote.notes, status: quote.status } : null,
    client,
  ...
  const handleValidateAll = () => {
    ...
    const updated = { ...templateData, sections: templateData.sections.map((s) => ({
  ...
  const handleReset = () => setTemplateData(getDefaultTemplate(currentTemplateType));
    ...
    return (
      <div className="p-4 lg:p-8 max-w-5xl mx-auto">
        <TemplateEditor
          ...
          onSave={(s) => {
            saveTemplate.mutate({ type: currentTemplateType, structure: s });
            setTemplateData(s);
          ...
          {view === "client" ? (
            ...
            <div className="printable-area bg-white text-black min-h-[800px] shadow-lg rounded-xl overflow-hidden">
              ...
              <div className="p-10 space-y-10">
                ...
                {/* Template sections */}
                {templateData && templateData.sections.map((section, sIdx) => (
            ...
            <div className="printable-area bg-white text-black min-h-[800px] shadow-lg rounded-xl overflow-hidden">
              ...
              <div className="p-8 space-y-8">
                ...
                {/* Template sections for manager view */}
                {templateData && templateData.sections.map((section, sIdx) => (
                  <TemplateSectionRenderer
                    key={sIdx}
                    section={section}
                ...
                {/* Staff section (kept from original) */}
                <section>
                  <h3 className="font-black text-sm uppercase text-slate-400 mb-3 flex items-center gap-2">
      ...
      {showImport && (
        <ImportTemplateModal
          ...
          onImported={(structure, filename) => {
            saveTemplate.mutate({ type: currentTemplateType, structure, sourceFilename: filename });
